...
void MatrixMulOnDevice(const Matrix M, const Matrix N, Matrix P)
{
    ...
    dim3 dim_block((P.width+TILE_WIDTH-1)/TILE_WIDTH, (P.height+TILE_WIDTH-1)/TILE_WIDTH, 1), dim_grid(TILE_WIDTH,TILE_WIDTH,1);

    // Launch the device computation threads!

    MatrixMulKernel<<<dim_block, dim_grid>>>(Md, Nd, Pd);
    cudaError_t cuda_ret = cudaDeviceSynchronize();
    if (cudaSuccess != cuda_ret) {
        printf("Unable to execute the kernel");
        exit(0);
    }
